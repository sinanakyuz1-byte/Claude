...
const PROVIDER = 'OEDAŞ';
const PROVINCES = ['Zonguldak', 'Bartın', 'Karabük'];

async function scrape() {
  const allOutages = [];
  for (const province of PROVINCES) {
    try {
      const html = await fetchHtml(`https://www.oedas.com.tr/elektrik-kesintileri?il=${encodeURIComponent(province)}`);
      allOutages.push(...parseHtml(html, province));
    } catch (err) {
      console.warn(`[OEDAŞ] Error for ${province}:`, err.message);
    }
  }
  return allOutages;
}
...
module.exports = { scrape, name: 'OEDAŞ', provinces: PROVINCES };
