...
const PROVIDER = 'TREDAŞ';
const PROVINCES = ['Çanakkale', 'Edirne', 'Kırklareli', 'Tekirdağ'];

async function scrape() {
  const allOutages = [];
  for (const province of PROVINCES) {
    try {
      const html = await fetchHtml(`https://www.tredas.com.tr/elektrik-kesintileri?il=${encodeURIComponent(province)}`);
      allOutages.push(...parseHtml(html, province));
    } catch (err) {
      console.warn(`[TREDAŞ] Error for ${province}:`, err.message);
    }
  }
  return allOutages;
}
...
module.exports = { scrape, name: 'TREDAŞ', provinces: PROVINCES };
