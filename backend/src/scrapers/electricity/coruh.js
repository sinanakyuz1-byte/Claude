/**
 * Çoruh EDAŞ - Artvin, Gümüşhane, Rize, Trabzon, Bayburt
 */
const { fetchHtml, normalizeText, makeOutage, cheerio } = require('../scraper-utils');

const PROVIDER = 'Çoruh EDAŞ';
const PROVINCES = ['Artvin', 'Gümüşhane', 'Rize', 'Trabzon', 'Bayburt'];

async function scrape() {
  const allOutages = [];
  for (const province of PROVINCES) {
    try {
      const html = await fetchHtml(`https://www.coruhedas.com.tr/elektrik-kesintileri?il=${encodeURIComponent(province)}`);
      allOutages.push(...parseHtml(html, province));
    } catch (err) {
      console.warn(`[Çoruh EDAŞ] Error for ${province}:`, err.message);
    }
  }
  return allOutages;
}
...
module.exports = { scrape, name: 'Çoruh EDAŞ', provinces: PROVINCES };
