/**
 * Gediz EDAŞ - İzmir, Manisa, Afyonkarahisar, Kütahya, Uşak
 */
const { fetchHtml, fetchJson, normalizeText, makeOutage, cheerio } = require('../scraper-utils');

const PROVIDER = 'Gediz EDAŞ';
const PROVINCES = ['İzmir', 'Manisa', 'Afyonkarahisar', 'Kütahya', 'Uşak'];
...
async function scrape() {
  ...
  for (const province of PROVINCES) {
    try {
      ...
      console.warn(`[Gediz EDAŞ] Error for ${province}:`, err.message);
    }
  }
  return allOutages;
}
...
async function scrapeProvince(province) {
  try {
    const data = await fetchJson(
      `https://www.gedizedas.com.tr/api/kesinti?il=${encodeURIComponent(province)}`,
      { headers: { Referer: 'https://www.gedizedas.com.tr' } }
    );
  ...
  const html = await fetchHtml(`https://www.gedizedas.com.tr/elektrik-kesintileri?il=${encodeURIComponent(province)}`);
...
module.exports = { scrape, name: 'Gediz EDAŞ', provinces: PROVINCES };
