 ...
 * Amasya, Çorum, Giresun, Kastamonu, Ordu, Samsun, Sinop
 */
const { fetchHtml, fetchJson, normalizeText, makeOutage, cheerio } = require('../scraper-utils');

const PROVIDER = 'YEDAŞ';
const PROVINCES = ['Amasya', 'Çorum', 'Giresun', 'Kastamonu', 'Ordu', 'Samsun', 'Sinop'];
...
async function scrape() {
  ...
  for (const province of PROVINCES) {
    try {
      ...
      console.warn(`[YEDAŞ] Error for ${province}:`, err.message);
    }
  }
  return allOutages;
}
...
async function scrapeProvince(province) {
  try {
    const data = await fetchJson(`https://www.yedas.com.tr/api/kesinti?il=${encodeURIComponent(province)}`, {
      headers: { Referer: 'https://www.yedas.com.tr' },
    });
  ...
  const html = await fetchHtml(`https://www.yedas.com.tr/elektrik-kesintileri?il=${encodeURIComponent(province)}`);
...
module.exports = { scrape, name: 'YEDAŞ', provinces: PROVINCES };
