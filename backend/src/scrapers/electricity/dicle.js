/**
 * Dicle EDAŞ - Diyarbakır, Şanlıurfa, Mardin, Batman, Siirt, Şırnak, Adıyaman
 */
const { fetchHtml, fetchJson, normalizeText, makeOutage, cheerio } = require('../scraper-utils');

const PROVIDER = 'Dicle EDAŞ';
const PROVINCES = ['Diyarbakır', 'Şanlıurfa', 'Mardin', 'Batman', 'Siirt', 'Şırnak', 'Adıyaman'];

async function scrape() {
  const allOutages = [];
  for (const province of PROVINCES) {
    try {
      const html = await fetchHtml(`https://www.dicleedas.com.tr/elektrik-kesintileri?il=${encodeURIComponent(province)}`);
      allOutages.push(...parseHtml(html, province));
    } catch (err) {
      console.warn(`[Dicle EDAŞ] Error for ${province}:`, err.message);
    }
  }
  return allOutages;
}
...
module.exports = { scrape, name: 'Dicle EDAŞ', provinces: PROVINCES };
